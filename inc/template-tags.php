...
if ( ! function_exists( 'development_posted_on' ) ) :
	...
	function development_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'development' ),
...
if ( ! function_exists( 'development_posted_by' ) ) :
	...
	function development_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'development' ),
	...
	function development_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'development' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
